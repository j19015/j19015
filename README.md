...
[![Blog](https://img.shields.io/badge/Blog-kt--tech.blog-9333EA?style=for-the-badge&logo=hashnode&logoColor=white)](https://kt-tech.blog)
[![Qiita](https://img.shields.io/badge/Qiita-@takakou-55C500?style=for-the-badge&logo=qiita&logoColor=white)](https://qiita.com/takakou)
...
![Focus](https://img.shields.io/badge/Focus-Full%20Stack%20%C3%97%20AI%20Agents-181717?style=for-the-badge&color=9333EA)
...
<img align="right" width="380" src="https://raw.githubusercontent.com/j19015/j19015/output/github-contribution-grid-snake-dark.svg" />

静岡県浜松市からフルリモートで働く Web エンジニアです。**React / Next.js / TypeScript を中心にしたフロントエンド開発** を主戦場にしつつ、時期やプロダクトによっては **Node.js / Python(FastAPI) のバックエンド API** や、**AWS / Azure のクラウド構築** 側に寄って動いてきました。

最近は、**AI エージェント（Claude Code）を個人の効率化で終わらせず、チームで同じ品質で使える形に落とし込むこと** に関心があります。詰まったことや設計した内容は [kt-tech.blog](https://kt-tech.blog) や [Qiita](https://qiita.com/takakou) にアウトプットしています。
...
const koki = {
  ...
  currentlyWorkingOn: [
    "スクラムチームでのプロダクト開発 (KDDIアジャイル開発センター)",
    "社内向け LLM / RAG 検索ツール (業務委託)",
    "Azure OpenAI を使ったドキュメントレビュー基盤 (業務委託)",
  ],
  recentlyExploring: [
    "Claude Code の skill / subagent 設計とセッション間連携",
    "ターミナル環境の多重化 (tmux / herdr / cmux)",
...
### 1️⃣ フロントを軸に、必要な範囲まで自分で取りに行けること
フロントエンドを主戦場にしつつ、時期とプロダクトによってはバックエンド API やインフラ側に寄って動いてきました。API 設計を意識した実装、サーバーレス構成の設計、IaC までひと通り自分で触れます。

### 2️⃣ 開発を回す仕組みをつくること
規約はドキュメントに書くだけだと、人が増えた瞬間に崩れます。読まないからではなく、読んでも忘れるからです。だから **Lint ルールや CI として機械的に強制される形** に落とすことを重視しています。ブランチ戦略・レビュールールの策定、pre-commit / GitHub Actions の整備、clone 直後に動く開発環境づくりなど。

### 3️⃣ 0→1 のプロダクト開発経験
個人事業主としてヒアリング・要件定義・技術選定・実装・運用まで単独で完遂した経験。業務委託でも立ち上げフェーズから参画し、技術選定から運用まで伴走しています。

### 4️⃣ AI エージェントを開発に組み込むこと
Claude Code を業務・個人開発の両方で常用。skill / subagent の設計、複数セッションの並行運用、定型作業の自動化などを実践し、その設計と落とし穴を [ブログ](https://kt-tech.blog) にまとめています。
...
| **Testing** | ![Vitest](https://img.shields.io/badge/Vitest-6E9F18?style=flat-square&logo=vitest&logoColor=white) ![Jest](https://img.shields.io/badge/Jest-C21325?style=flat-square&logo=jest&logoColor=white) ![Testing%20Library](https://img.shields.io/badge/Testing%20Library-E33332?style=flat-square&logo=testinglibrary&logoColor=white) ![Playwright](https://img.shields.io/badge/Playwright-2EAD33?style=flat-square&logo=playwright&logoColor=white) |
...
| Product | 概要 | Stack |
|:---|:---|:---|
| **[kt-tech.blog](https://kt-tech.blog)** | Notion を CMS にした個人技術ブログ。図解パイプラインや OGP 生成も自作 | `Next.js` `Cloudflare Pages` `Notion API` `R2` |
| **[con-cafe.jp](https://con-cafe.jp)** | 全国コンセプトカフェマップ（株式会社Diafa での担当プロダクト） | `Next.js` `Prisma` `PostgreSQL` `AWS` |
| **[anoni.space](https://anoni.space)** | 知人と共同運営中のプロダクト | `Next.js` `Cloudflare` |

### 📦 Repositories

| Repository | 概要 | Stack |
|:---|:---|:---|
| **[kt-tech.blog](https://github.com/j19015/kt-tech.blog)** | 上記ブログのソース。Notion → 静的生成、SVG 図解、アイキャッチ自動生成 | `TypeScript` `Next.js` |
| **[react-vite-template](https://github.com/j19015/react-vite-template)** | SPA を立ち上げるときの自分用テンプレート | `React` `Vite` `TypeScript` |
| **[lgtm-generator](https://github.com/j19015/lgtm-generator)** | LGTM 画像をブラウザだけで生成する | `HTML` `JavaScript` |
| **[MicroCMS_Markdown_Editor](https://github.com/j19015/MicroCMS_Markdown_Editor)** | microCMS 用の Markdown エディタ | `TypeScript` |
| **[Next_Go_Bookers2](https://github.com/j19015/Next_Go_Bookers2)** | Next.js + Go の認証付き本管理アプリ | `Next.js` `Go` |
| **[AWS_CDK_Workshop](https://github.com/j19015/AWS_CDK_Workshop)** | AWS CDK Workshop を写経したときのリポジトリ | `TypeScript` `AWS CDK` |
...
普段は **[kt-tech.blog](https://kt-tech.blog)** で、実務で詰まったこと・設計したこと・触ってみた技術を書いています。
（以下は GitHub Actions が RSS から毎日自動更新しています）

<!-- BLOG-POST-LIST:START -->
- [自宅のWindowsを開発機にする その5: Orcaで仕事用アカウントと会話履歴を正しく扱う](https://kt-tech.blog/blogs/wsl-devbox-5-orca-accounts-history)
- [自宅のWindowsを開発機にする その4: Orcaを入れてWSLのリポジトリを扱う](https://kt-tech.blog/blogs/wsl-devbox-4-orca)
- [自宅のWindowsを開発機にする その3: Claude Codeの設定とメモリを引っ越す](https://kt-tech.blog/blogs/wsl-devbox-3-claude-code-config)
- [自宅のWindowsを開発機にする その2: Macの開発環境を作業状態ごとWSLに移す](https://kt-tech.blog/blogs/wsl-devbox-2-migrate-env)
- [自宅のWindowsを開発機にする その1: TailscaleでMacとスマホからWSLに入る](https://kt-tech.blog/blogs/wsl-devbox-1-tailscale)
...
- DX (Developer Experience) の向上・開発基盤の整備
- 技術選定・アーキテクチャ設計
- AI / LLM を活用したプロダクト開発
- リアルタイム通信・ストリーミング技術
...
[![Blog](https://img.shields.io/badge/Blog-kt--tech.blog-9333EA?style=for-the-badge&logo=hashnode&logoColor=white)](https://kt-tech.blog)
[![GitHub](https://img.shields.io/badge/GitHub-j19015-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/j19015)
[![Qiita](https://img.shields.io/badge/Qiita-takakou-55C500?style=for-the-badge&logo=qiita&logoColor=white)](https://qiita.com/takakou)
[![X](https://img.shields.io/badge/X-tech__koki-000000?style=for-the-badge&logo=x&logoColor=white)](https://x.com/tech_koki)
[![Wantedly](https://img.shields.io/badge/Wantedly-kt__tech-00A4BB?style=for-the-badge&logo=wantedly&logoColor=white)](https://www.wantedly.com/id/kt_tech)
[![YOUTRUST](https://img.shields.io/badge/YOUTRUST-meow__koki-00A0E9?style=for-the-badge&logoColor=white)](https://youtrust.jp/users/meow_koki)
